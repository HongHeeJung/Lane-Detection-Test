...
# Possibly change the Region of interest vertices
video = cv2.VideoCapture('C:/Users/hongh/Pictures/project/Drive.mp4')
...
def roi(video, vertices):
    ...
    cv2.fillPoly(masks, vertices, 255)
    video_roi = cv2.bitwise_and(video, masks)
    return video_roi
...
while True:
    ...
    # Canny
    edges = cv2.Canny(target, 50, 70)

    # Region of interest points
    vertices = np.array([[200, 750], [450, 650], [450, 650], [600, 525], [750, 525], [1250, 750]
                         ], np.int32)
    ...
    # Resize
    frameR = cv2.resize(line_image, (1280, 720))
